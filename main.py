...
if __name__ == "__main__":
    ...
    receiver_pos = torch.tensor([[-0.3, -0],[-0.1, -0], [0.1, -0]])
    ...
    initial_guess = torch.tensor([[-1., 1]])
    initial_guess = torch.tensor([[1., 0.1]])
    initial_guess = torch.tensor([[-1., 0.1]])
    initial_guess = torch.tensor([[-0.5, 0.5]])
    initial_guess = torch.tensor([[0., 0.5]])
    initial_guess = torch.tensor([[0.5, 0.5]])
    initial_guess = torch.tensor([[0.9, 0.9]])

    #initial_guess = torch.tensor([[0.3, 0.5],[-0.3, 0.5],[0.,0.8]])
    ...
    A = 100.
    T = 0.01 #time window not period
    c = 343.0
    sigma = 0.0001
    f = 20000
    samplerate = 96000

    simulator = SoundSimulator(A=A, f=f, c=c, sigma=sigma, T=T, samplerate=samplerate, sender_pos=sender_pos,
                               receiver_pos=receiver_pos)
    signals = simulator.simulate_echoes(object_pos=obj_pos).detach()
    utils.plot_signals(signals, title="True signals")
    exit()
    predict_positions = simulator.predict_position(signals, plot=True, true_obj=obj_pos, n_points= obj_pos.shape[0])
